...
EXP <- read.table( synapser::synGet('syn21292032')$path, row.names=1, header=T, sep='\t' )
#Remove Leading X from column names
colnames(EXP) <- gsub( "X", "", colnames(EXP) )

Meta <- read.csv( synapser::synGet('syn3191087')$path, header=T, stringsAsFactors = F)
Meta2 <- read.csv( synapser::synGet('syn21323366')$path, header=T, stringsAsFactors = F )

ID_Key <- read.csv( synapser::synGet('syn3382527')$path )
...
```{r SetAnaylsisParameters }
##########################################################################################
#Logistic Ordinal Regression...
#GN <- colnames(EXP)[1985]
#Path <- Path
#Exp <- EXP 
#NeuroPath_Calc( GN,Path,Exp)
...
thisFileName <- 'NeuroPath_Regression.Rmd'
# Github link
...
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='Neuropathology_Scores.csv', name = 'ROSMAP DLPFC NeuroPath Regression Results', parentId=CODE$properties$id ), activityName = activityName, executed = c( Syns_Used, thisFile) , activityDescription = activityDescription)
...
setwd("~/NeuroPath_Regression/code/")
...
createAndKnitToFolderEntityClient(file = "NeuroPath_Regression.Rmd",
                                          parentId ="syn7525089",
